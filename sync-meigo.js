/**
 * 功能：自动更新版本号
 * 命令：node sync-meigo.js
 * 实现：获取远端的meigo.json文件，拿到上次发布后的版本号，
 *      自动增加一，然后生成一个新的meigo.json文件，
 *      再次发布会使用新的替换旧的文件，实现版本号自动增加
 * 更新文件：./build/meigon.json ./package.json ./README.md
 */
...
var options = {
    hostname: '114.55.114.18',//以ip代替域名，方便本地的host随意更改
    port: 80,
    path: '/meigo.json',
    method: 'GET',
    headers: {
...
var req = http.request(options, (res) => {
    ...
    res.on('data', (chunk) => {
        ...
        try {
            ...
            fs.writeFileSync('./build/meigo.json', prettyJson(meigo), 'utf8');
